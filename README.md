...
The Netlify build exports static HTML, CSS, JavaScript and assets. Do not deploy `dist/server` to Netlify. The static export includes all three dynamic destination slugs through `generateStaticParams`. No backend, payment processing, form storage, private authentication or API keys are required by the website.

## Hosting independence

This repository builds for Netlify without OpenAI or Cloudflare hosting configuration. `npm run build` and `npm run build:netlify` both produce the static site in `dist/client`. `npm start` previews it locally. The previous Sites deployment is not updated by this repository; its private access settings do not transfer to Netlify.
